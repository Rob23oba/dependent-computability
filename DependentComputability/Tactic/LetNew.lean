...
partial def letNewHelper (goalType : Expr) (val : Expr) (baseMap : FVarIdMap Expr) :
  ...
  | .letE nm t v b false =>
    ...
    withLetDecl nm t v fun var_base => do
    withLetDecl (nm.appendAfter "_extra") (mkExtraApp newType var_base) newValue fun var => do
  ...
  | .letE nm t v b true =>
    let newType ← conversionStepNew.visit t baseMap
    let newValue ← conversionStepNew.visit v baseMap
    withLocalDeclD nm t fun var_base => do
    withLocalDeclD (nm.appendAfter "_extra") (mkExtraApp newType var_base) fun var => do
      let (res, mvar) ← letNewHelper goalType (b.instantiate1 var_base)
        (baseMap.insert var_base.fvarId! var)
      return (mkApp2 (← mkLambdaFVars #[var_base, var] res) v newValue, mvar)
  | .lam nm t b bi =>
    let newType ← conversionStepNew.visit t baseMap
    withLocalDecl nm bi t fun var_base => do
    withLocalDecl (nm.appendAfter "_extra") bi (mkExtraApp newType var_base) fun var => do
      let (res, mvar) ← letNewHelper goalType (b.instantiate1 var_base)
        (baseMap.insert var_base.fvarId! var)
